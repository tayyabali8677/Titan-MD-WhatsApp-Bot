...
class Client {
  ...
  async _onMessage(raw) {
    ...
    // Run before-message middleware (anti-link, anti-spam, auto-reply, etc.)
    // Any handler returning false short-circuits — message is not dispatched.
    const { getBeforeMessageHandlers } = require('./plugins');
    for (const h of getBeforeMessageHandlers()) {
      try {
        const cont = await h(msg, this.sock);
        if (cont === false) return;
      } catch (e) { logger.error({ err: e.message, where: 'beforeMessage' }); }
    }
