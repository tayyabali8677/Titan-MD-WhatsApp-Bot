...
async function createConnection() {
  ...
  let _hasSentStartup = false;
  sock.ev.on('connection.update', async (update) => {
    if (update.connection === 'open') {
      console.log('[titan-md] WhatsApp connected.');

      // ── Startup message — DM the owner that the bot is online.
      // Skipped if DISABLE_START_MESSAGE=true, and de-duped within the
      // same process so a reconnect doesn't double-send.
      if (_hasSentStartup) return;
      if (String(config.DISABLE_START_MESSAGE || 'false').toLowerCase() === 'true') return;
      _hasSentStartup = true;
      try {
        const ownerJid = (config.OWNER_NUMBER ? config.OWNER_NUMBER.replace(/\D/g, '') + '@s.whatsapp.net' : null)
          || sock.user?.id;
        if (!ownerJid) return;
        const { getPluginsCount } = require('./plugins');
        const pluginsCount = getPluginsCount();
        const text =
          `*⚡ Titan MD is online!*\n\n` +
          `📦 Version: v${config.VERSION}\n` +
          `🧩 Plugins: ${pluginsCount}\n` +
          `🕐 Boot time: ${new Date().toLocaleString('en-US', { timeZone: config.TZ || 'UTC' })}\n` +
          `🌐 Platform: ${process.platform}\n\n` +
          `_Send_ \`.help\` _to see all commands._`;
        await sock.sendMessage(ownerJid, { text });
      } catch (e) {
        console.error('[titan-md] startup-message failed:', e.message);
      }
    }
  ...
  // Group-participants events (welcome / goodbye / anti-bot / anti-promote)
  sock.ev.on('group-participants.update', async (event) => {
    try {
      const { getGroupEventHandlers } = require('./plugins');
      for (const h of getGroupEventHandlers()) {
        try { await h(event, sock); }
        catch (e) { console.error('[titan-md] group event handler error:', e.message); }
      }
    } catch (_) {}
  });
